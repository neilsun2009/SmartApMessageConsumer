...
public class CollectionHotspotConsumer extends Thread{
	...
	private final String TOPIC = "COLLECTION_HOTSPOT";
	
	private final String DATA_TYPE = "SMART_AP_1002";
    ...
    public CollectionHotspotConsumer() {
    ...
    private void writeIntoTable(List<String> message) throws Exception {
		...
		for (SourceFilesHelper.DataTypeProperties item : SourceFilesHelper.DataTypeProperties
			...
			if (item.getIdentifier() != null
					&& DATA_TYPE.contains(item.getIdentifier())) {
				...
				for(int i = 0 ; i < datas.size(); i++) {
					...
					String sql = null;
					String tableName = TableInsertParameter.collectionHotspotTableName;
				...
				try {
						if(pool.execBatchSql(sqlList))
							System.out.println("COLLECTION_HOTSPOT：WriteIntoNormalTableSucceed");
						else
							System.out.println("COLLECTION_HOTSPOT：WriteIntoNormalTableFailed");
